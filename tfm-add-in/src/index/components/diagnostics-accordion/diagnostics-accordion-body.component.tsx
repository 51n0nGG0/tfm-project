...
const DiagnosticsAccordionBody: React.FC<DiagnosticsAccordionBodyProps> = ({diagnosticsList}) => {
    return(
        <Accordion multiple collapsible >
            { diagnosticsList?.map((diagnostic:DiagnosticItem, index:number) => 
                <DiagnosticsAccordionItem value={index} name={diagnostic.name} description={diagnostic.shortDescription}/>
            )}
        </Accordion>
    );
}
